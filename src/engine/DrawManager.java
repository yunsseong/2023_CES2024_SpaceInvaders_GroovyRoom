...
public final class DrawManager {
    ...
    public static int selectedCustom;
    ...
    private DrawManager() {
        ...
        try {
            ...
            /** 커스터마이징
             spriteMap.put(SpriteType.CustomShip1, new boolean[커스텀_행][커스텀_열]);
             spriteMap.put(SpriteType.CustomShip2, new boolean[커스텀_행][커스텀_열]);
             **/

            fileManager.loadSprite(spriteMap);
            fileManager.changeSprite(spriteMap, SpriteType.CustomShip1, 0);
            fileManager.changeSprite(spriteMap, SpriteType.CustomShip2, 0);
     ...
     *
     * @param screen Screen to draw on.
     * @param lives  Current player1's lives.
     */
     ...
     *
     * @param screen Screen to draw on.
     * @param lives2 Current player2's lives.
     */
     ...
     * Draws number of items currently in inventory on screen.
     *
     * @param magazine     Number of remaining magazines
     * @param bullet_count Number of bullets fired
     * @param screen       Screen to draw on.
     */
    ...
    public void drawMenu(final Screen screen, final int option) {
        ...
        String highScoresString = "HIGH SCORE";
        String shopString = "SHOP";
        String settingString = "SETTING";
        String customString = "CUSTOMIZING";
        ...
        if (option == 7)
            ...
            backBufferGraphics.setColor(Color.WHITE);
        drawCenteredRegularString(screen, customString, screen.getHeight() / 3
        ...
        drawCenteredRegularString(screen, exitString, screen.getHeight() / 3
                * 2 + fontRegularMetrics.getHeight() * 9 - fontRegularMetrics.getHeight() * 2);  // 위치 조정
    }
     ...
     * @param livesRemaining1 1p's lives remaining when finished.
     ...
     * @param score           Score obtained.
     * @param livesRemaining1 1p's lives remaining when finished.
     * @param livesRemaining2 2p's lives remaining when finished.
     * @param shipsDestroyed  Total ships destroyed.
     * @param accuracy        1p's accuracy.
     * @param accuracy2       2p's accuracy.
     * @param isNewRecord     If the score is a new high score.
     */
    public void drawResults(final Screen screen, final int score,
    ...
    public void drawSettingDetail(final Screen screen, final int option, final boolean selected,
        ...
        if (option == 0 || option == 1) {
            ...
            if (bgmOn)
                drawSevenTenthRegularString(screen, "ON", screen.getHeight() / 4 + fontRegularMetrics.getHeight() * 4);
            else
                drawSevenTenthRegularString(screen, "OFF", screen.getHeight() / 4 + fontRegularMetrics.getHeight() * 4);
        }
    ...
    private void drawSprite(Graphics g, int x, int y, int size, boolean[][] grid, Color color) {
        g.setColor(color);
        for (int row = 0; row < grid.length; row++) {
            for (int col = 0; col < grid[row].length; col++) {
                if (grid[row][col]) {
                    g.fillRect(x + row * size / grid.length, y + col * size / grid[row].length, size / grid.length, size / grid[row].length);
                }
            }
        }
    }
    public void drawSelectCustom(final Screen screen, int selectedBox) {
        ...
        // Draw the title
        String title = "SELECT YOUR SHIP DESIGN!";
        int titleWidth = backBufferGraphics.getFontMetrics().stringWidth(title);
        int titleX = (screen.getWidth() - titleWidth) / 2;
        int titleY = 80;
        ...
        // Adjust box size and spacing
        int boxSize = 40; // Smaller size for the boxes
        int boxSpacing = 25; // Bigger space between boxes

        int boxX = (screen.getWidth() - boxSize) / 2;
        int boxY = titleY + backBufferGraphics.getFontMetrics().getHeight() + boxSpacing / 3;
        ...
        for (int i = 0; i < 6; i++) {
            ...
            // Draw sprite if there is a custom ship at the current index
            if (i < customShips.size()) {
                Map.Entry<boolean[][], Color> customShip = customShips.get(i);
                boolean[][] grid = customShip.getKey();
                Color color = customShip.getValue();

                // Draw the sprite using grid and color
                drawSprite(backBufferGraphics, boxX, boxY + i * (boxSize + boxSpacing), boxSize, grid, color);
            }
        }
    }
    ...
    public void drawCustomizing(final CustomizeScreen screen, int x_postion, int y_postion, Color[][] filledColors) {
        ...
        int paletteStartY = screenHeight - paletteHeight + 20;
        int borderThickness = 2;
        Map.Entry<boolean[][], Color> customShip = null;

        List<Map.Entry<boolean[][], Color>> loadedList = FileManager.loadSkinList();
        ...
        if (customShip == null) {
            for (int i = 0; i < 10; i++) {
                for (int j = 0; j < 10; j++) {
                    boolean tmp = i >= 3 && i < 7 && j >= 3 && j < 7;
                    if (tmp) isFilled[i][j] = true;
                }
            }
        }else{
...
// Set the default color for subsequent drawings
        ...
        int totalWidth = dotCount * boxSize;
        int totalHeight = dotCount * boxSize;
